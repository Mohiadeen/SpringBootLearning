...
public class JerseyClientExamples 
{
	...
	private static void httpGETCollectionExample() 
	{
		...
		WebTarget webTarget = client.target("http://localhost:8080/JerseyFilter/rest").path("employees");
		...
		WebTarget baseTarget = client2.target("http://localhost:8080/JerseyFilter/rest");
		WebTarget empTarget = baseTarget.path("employees");

		Response response2 = empTarget.request(MediaType.APPLICATION_JSON).get();

		List<Employee> list = response2.readEntity(new GenericType<List<Employee>>() {
		});
	...
	private static void httpGETEntityExample() 
	{
		...
		WebTarget webTarget = client.target("http://localhost:8080/JerseyFilter/rest").path("employees").path("1");
